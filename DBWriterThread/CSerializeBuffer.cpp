...
MemoryPool_TLS<CSerializeBuffer>* CSerializeBuffer::m_bufferPool = new MemoryPool_TLS<CSerializeBuffer>(true,TlsAlloc());
BYTE CSerializeBuffer::m_packetCode;
BYTE CSerializeBuffer::m_packetKey;

CSerializeBuffer::CSerializeBuffer()
{
	pBuffer = new char[DEFAULT_MAX_BUFFSIZE];
...
WORD CSerializeBuffer::GetPayloadLen()
{
...
bool CSerializeBuffer::Decode(bool* recvEndCode = nullptr)
{
	// 1. CheckCode 확인
	BYTE code = pBuffer[CODE_INDEX];
	if (code != m_packetCode)
		return false;
	...
	for (int i = 0; i < repeatTime; ++i)
	{
		p = pBuffer[i + 4] ^ (lastE + m_packetKey + i + 1);
		lastE = pBuffer[i + 4];
...
void CSerializeBuffer::PutHeaderData(BYTE * pHeader, bool bHeaderSize5Byte)
{
